...
class Passwords:
    ...
    def check_if_account_name_already_exists(self, account: str):
        ...
        accounts_with_parameter_name_quantity = QueryExecutor().simple_consult_query(
            query=accounts_with_parameter_name_query, params=query_values)
        accounts_with_parameter_name_quantity = QueryExecutor().treat_simple_result(
            value_to_treat=accounts_with_parameter_name_quantity, values_to_remove=to_remove_list)
    ...
    def get_user_passwords_quantity(self):
        ...
        user_passwords_quantity = QueryExecutor().simple_consult_query(
            check_user_passwords_quantity_query, params=(logged_user, logged_user_password))
        user_passwords_quantity = QueryExecutor().treat_simple_result(
            user_passwords_quantity, to_remove_list)
    ...
    def get_user_accounts_names(self):
        ...
        accounts = QueryExecutor().complex_consult_query(
            query=search_accounts_query, params=(logged_user, logged_user_password))
    ...
    def create_new_password(self):
        ...
        with col1:
            ...
            with st.expander(label="Dados", expanded=True):
                ...
                login = st.text_input(
                    label='Login', help="Seu usuário no site")
                password = st.text_input(
                    label="Senha", type="password", help="Sua senha do site")
            ...
            if send_button and confirm_values == True:
                ...
                if site != '' and url != '' and login != '' and password != '':
                    ...
                    if is_name_available:
                        ...
                        query_values = (site, url, login, password,
                                        logged_user_name, logged_user_document)

                        QueryExecutor().insert_query(query=insert_password_query, values=query_values,
                                                     success_message='Senha cadastrada com sucesso!', error_message='Erro ao cadastrar senha:')

                        log_query = '''INSERT INTO logs_atividades (usuario_log, tipo_log, conteudo_log) VALUES(%s, %s, %s)'''
                        log_query_values = (logged_user, 'Cadastro', 'Cadastrou a senha {} associada ao email {}'.format(
                            query_values[0], query_values[3]))
    ...
    def read_password(self):
        ...
        if user_passwords_quantity == 0:
            ...
            result_list = QueryExecutor().complex_consult_query(
                query=account_details_query, params=account_details_values)
            ...
            if confirm_selection and consult_button:
                ...
                if safe_password != "" and confirm_safe_password != "" and safe_password == confirm_safe_password and is_password_valid == True:
                    ...
                    with col2:
                        ...
                        with st.expander(label="Dados", expanded=True):
                            ...
                            log_query = '''INSERT into logs_atividades (usuario_log, tipo_log, conteudo_log) VALUES(%s, %s, %s)'''
                            query_values = (
                                logged_user, 'Consulta', 'Consultou a senha do site {}'.format(selected_option))
                    ...
                    with col2:
                        ...
                        with data_validator_expander:
                            ...
                            if confirm_safe_password == '':
                                st.error(
                                    body="A confirmação da senha não foi preenchida.")
                ...
                with col3:
                    ...
                    with data_validator_expander:
                        st.warning(
                            body="Confirme a seleção antes de realizar a consulta.")
                ...
                if safe_password != "" and confirm_safe_password != "" and safe_password == confirm_safe_password and is_password_valid == True:
                    ...
                    with col2:
                        ...
                        if confirm_values and send_button:
                            ...
                            update_site_values = (
                                url, login, password, selected_option, logged_user_name, logged_user_document)

                            QueryExecutor().insert_query(query=update_site_query, values=update_site_values,
                                                         success_message="Senha atualizada com sucesso!", error_message="Erro ao atualizar senha:")

                            log_query = '''INSERT into logs_atividades (usuario_log, tipo_log, conteudo_log) VALUES(%s, %s, %s)'''
                            query_values = (
                                logged_user, 'Atualização', 'Atualizou a senha do site {}.'.format(selected_option))
                            QueryExecutor().insert_query(query=log_query, values=query_values,
                                                         success_message='Log gravado.', error_message='Erro ao gravar log:')
                ...
                if safe_password != "" and confirm_safe_password != "" and safe_password == confirm_safe_password and is_password_valid == True:
                    ...
                    if confirm_delete_selection and delete_password_button:
                        with col2:
                            ...
                            QueryExecutor().insert_query(query=delete_password_query, values=delete_password_values,
                                                         success_message="Senha excluída com sucesso!", error_message="Erro ao excluir senha:")

                            log_query = '''INSERT into logs_atividades (usuario_log, tipo_log, conteudo_log) VALUES(%s, %s, %s)'''
                            query_values = (
                                logged_user, 'Exclusão', 'Excluiu a senha do site {}'.format(selected_option))
                            QueryExecutor().insert_query(query=log_query, values=query_values,
                                                         success_message='Log gravado.', error_message='Erro ao gravar log:')
                        ...
                        with col1:
                            ...
                            with data_validator_expander:
                                st.warning(
                                    body="Confirme a exclusão da senha.")
    ...
    def main_menu(self):
        ...
        with col2:
            menu_options = ["Cadastrar senha", "Consultar senha",
