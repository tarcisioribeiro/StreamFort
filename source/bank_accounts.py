...
class BankAccount:
    ...
    def check_if_bank_account_name_already_exists(self, bank_account_name: str):
        ...
        query_values = (bank_account_name, logged_user_name,
                        logged_user_document)

        bank_accounts_with_parameter_name_quantity = QueryExecutor().simple_consult_query(
            query=bank_accounts_with_parameter_name_query, params=query_values)
        bank_accounts_with_parameter_name_quantity = QueryExecutor().treat_simple_result(
            value_to_treat=bank_accounts_with_parameter_name_quantity, values_to_remove=to_remove_list)
        bank_accounts_with_parameter_name_quantity = int(
            bank_accounts_with_parameter_name_quantity)

        if bank_accounts_with_parameter_name_quantity == 0:
    ...
    def get_user_accounts_quantity(self):
        ...
        user_accounts_quantity = QueryExecutor().simple_consult_query(
            check_user_bank_accounts_query, params=(logged_user_name, logged_user_document))
        user_accounts_quantity = QueryExecutor().treat_simple_result(
            user_accounts_quantity, to_remove_list)
    ...
    def get_user_bank_accounts(self):
        ...
        bank_accounts = QueryExecutor().complex_consult_query(
            query=search_bank_accounts_query, params=(logged_user_name, logged_user_document))
        bank_accounts = QueryExecutor().treat_numerous_simple_result(
            bank_accounts, to_remove_list)
        ...
        with col1:
            with st.expander(label="Dados da Conta", expanded=True):
                account_name = st.text_input(label="Nome", max_chars=100, placeholder='Conta',
                                             help="Necessário para identificação. A sugestão é informar algo direto e descritivo, como por exemplo 'Conta Corrente BB'.")
                financial_institution = st.selectbox(
                    label='Instituição', options=financial_institution_list, help='Insituição financeira a qual pertence a conta.')
                financial_institution_code = st.text_input(
                    label="Código da instituição", max_chars=5, help="Código da insituição financiera no SPB (Sistemas de Pagamentos Brasileiro).")
                agency = st.text_input(
                    label="Agência", max_chars=10, help="Número da agência.")
        ...
        with col2:
            with st.expander(label="Dados da Conta", expanded=True):
                account_number = st.text_input(
                    label="Número da conta", max_chars=15, help="Número de identificação da conta.")
                account_digit = st.text_input(label="Dígito", max_chars=1, placeholder='0',
                                              help='Dígito identificador da conta. Caso não haja, preencha como 0.')
                account_password = st.text_input(label="Senha da conta", max_chars=30, type='password',
                                                 help='Senha utilizada para saques e demais operações em terminais físicos.')
                digital_account_password = st.text_input(
                    label="Senha digital da conta", max_chars=30, type='password', help='Senha digital da conta, utilizada para operações virtuais como Pix.')
            ...
            if confirm_data == True and register_new_account:
                ...
                is_bank_account_name_available = self.check_if_bank_account_name_already_exists(
                    bank_account_name=account_name)

                if is_bank_account_name_available:
                    with col3:
                        with st.expander(label="Validação de dados", expanded=True):
                    ...
                    query_values = (account_name, financial_institution, financial_institution_code, agency, account_number,
                                    account_digit, account_password, digital_account_password, logged_user_name, logged_user_document)

                    QueryExecutor().insert_query(insert_password_query, query_values,
                                                 'Conta cadastrada com sucesso!', 'Erro ao cadastrar conta:')

                    log_query = '''INSERT INTO logs_atividades (usuario_log, tipo_log, conteudo_log) VALUES(%s, %s, %s)'''
                    log_query_values = (
                        logged_user, 'Cadastro', 'Cadastrou a conta {}'.format(query_values[0]))
                    QueryExecutor().insert_query(query=log_query, values=log_query_values,
                ...
                with col3:
                    ...
                    with st.expander(label="Validação de dados", expanded=True):
                        st.warning(
                            body="Você deve confirmar os dados antes de prosseguir.")
    ...
    def read_bank_accounts(self):
        ...
        if user_accounts_quantity == 0:
            ...
            with col1:
                with st.expander(label="Dados", expanded=True):
                    selected_option = st.selectbox(
                        label="Selecione a conta", options=user_bank_accounts)
            ...
            result_list = QueryExecutor().complex_consult_query(query=account_details_query,
                                                                params=(selected_option, logged_user_name, logged_user_document))
            result_list = QueryExecutor().treat_complex_result(
                values_to_treat=result_list, values_to_remove=to_remove_list)
                ...
                if safe_password != "" and confirm_safe_password != "" and safe_password == confirm_safe_password and is_password_valid == True:
                    ...
                    with col2:
                        ...
                        with st.expander(label="Dados", expanded=True):
                            ...
                            log_query = '''INSERT into logs_atividades (usuario_log, tipo_log, conteudo_log) VALUES(%s, %s, %s)'''
                            query_values = (
                                logged_user, 'Consulta', 'Consultou a conta bancária {}'.format(selected_option))
                            QueryExecutor().insert_query(query=log_query, values=query_values,
                                                         success_message='Log gravado.', error_message='Erro ao gravar log:')
                ...
                if safe_password != "" and confirm_safe_password != "" and safe_password == confirm_safe_password and is_password_valid == True:
                    ...
                    with col2:
                        ...
                        with st.expander(label="Novos Dados", expanded=True):
                            ...
                            account_password = st.text_input(
                                label="Senha da conta", max_chars=30, type='password', help='Senha utilizada para saques e demais operações em terminais físicos.')
                            digital_account_password = st.text_input(
                                label="Senha digital da conta", max_chars=30, type='password', help='Senha digital da conta, utilizada para operações virtuais como Pix.')
                        ...
                        update_bank_account_button = st.button(
                            label=":arrows_counterclockwise: Atualizar dados da conta")
                    ...
                    if confirm_new_bank_account_data and update_bank_account_button and account_password != "" and digital_account_password != "":
                        ...
                        update_account_values = (
                            account_password, digital_account_password, selected_option, logged_user_name, logged_user_document)

                        QueryExecutor().insert_query(query=update_account_query, values=update_account_values,
                                                     success_message="Conta atualizada com sucesso!", error_message="Erro ao atualizar conta:")

                        log_query = '''INSERT into logs_atividades (usuario_log, tipo_log, conteudo_log) VALUES(%s, %s, %s)'''
                        query_values = (
                            logged_user, 'Atualização', 'Atualizou a conta bancária {}'.format(selected_option))
                        QueryExecutor().insert_query(query=log_query, values=query_values,
                                                     success_message='Log gravado.', error_message='Erro ao gravar log:')
                        ...
                        if delete_account_button and confirm_account_deletion:
                            ...
                            with col2:
                                ...
                                delete_account_values = (
                                    selected_option, logged_user_name, logged_user_document)

                                QueryExecutor().insert_query(query=delete_account_query, values=delete_account_values,
                                                             success_message="Conta excluída com sucesso!", error_message="Erro ao excluir conta:")

                                log_query = '''INSERT into logs_atividades (usuario_log, tipo_log, conteudo_log) VALUES(%s, %s, %s)'''
                                query_values = (
                                    logged_user, 'Exclusão', 'Excluiu a conta bancária {}'.format(selected_option))
                                QueryExecutor().insert_query(query=log_query, values=query_values,
                                                             success_message='Log gravado.', error_message='Erro ao gravar log:')
    ...
    def main_menu(self):
        ...
        with col2:
            menu_options = ["Cadastrar conta", "Consultar conta",
        ...
        if selected_option == menu_options[0]:
            self.create_new_bank_account()

        if selected_option == menu_options[1]:
            self.read_bank_accounts()
