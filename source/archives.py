...
class Archives:
    ...
    def check_if_archive_name_already_exists(self, archive_name: str):
        ...
        archives_with_parameter_name_quantity = QueryExecutor().simple_consult_query(
            query=cards_with_parameter_name_query, params=query_values)
        archives_with_parameter_name_quantity = QueryExecutor().treat_simple_result(
            value_to_treat=archives_with_parameter_name_quantity, values_to_remove=to_remove_list)
        archives_with_parameter_name_quantity = int(
            archives_with_parameter_name_quantity)
    ...
    def get_user_archives_quantity(self):
        ...
        user_archives_quantity = QueryExecutor().simple_consult_query(
            search_user_archives_quantity, params=(logged_user_name, logged_user_document))
    ...
    def get_archives_names(self):
        ...
        user_archives_name = QueryExecutor().complex_consult_query(
            search_user_archives_name, params=(logged_user_name, logged_user_document))
        user_archives_name = QueryExecutor().treat_numerous_simple_result(
            user_archives_name, to_remove_list)
        ...
        with col1:
            ...
            with st.expander(label="Entrada de Dados", expanded=True):
                archive_name = st.text_input(label="Nome do arquivo", max_chars=100,
                                             help="É necessário definir um nome para identificação e consulta posterior.")
                uploaded_file = st.file_uploader(label="Escolha um arquivo de texto", type=[
                                                 "txt"], help="São permitidos arquivos de texto, na extensão '.txt'. O tamanho do arquivo não pode exceder 200 MB.")
            ...
            if register_archive_button and uploaded_file is not None and content != "" and archive_name != "":
                is_archive_name_available = self.check_if_archive_name_already_exists(
                ...
                if is_archive_name_available:
                    ...
                    archive_values = (archive_name, content,
                                      logged_user_name, logged_user_document)

                    if content is not None:

                        QueryExecutor().insert_query(archive_query, archive_values,
                                                     "Upload do arquivo realizado com sucesso!", "Erro ao fazer upload do arquivo:")

                        log_query = """INSERT INTO logs_atividades (usuario_log, tipo_log, conteudo_log) VALUES(%s, %s, %s)"""
                        log_query_values = (
                            logged_user, "Cadastro", "Fez o upload do arquivo {}.".format(archive_name))
                        QueryExecutor().insert_query(log_query, log_query_values,
                    ...
                    with col2:
                        with st.expander(label="Validação dos dados", expanded=True):
                            st.error(
                                body="O nome do arquivo já está sendo utilizado.")
                ...
                with col2:
                    ...
                    if uploaded_file is None:
                        with st.expander(label="Validação dos dados", expanded=True):
                            st.error(
                                body="Não foi feito o upload de um arquivo.")
                    if archive_name == "":
                        with st.expander(label="Validação dos dados", expanded=True):
                            st.error(
                                body="Não foi informado um nome para o arquivo.")
                ...
                if consult_button and confirm_selection:
                    ...
                    if safe_password != "" and confirm_safe_password != "" and safe_password == confirm_safe_password and is_password_valid == True:
                        ...
                        with st.spinner(text="Aguarde..."):
                            ...
                            archive_content = (QueryExecutor().simple_consult_query(archive_content_query, params=(
                                selected_archive, logged_user_name, logged_user_document)))
                            archive_content = (QueryExecutor().treat_simple_result(
                                archive_content, to_remove_archive_list))
                ...
                with st.expander(label="Consulta", expanded=True):
                    ...
                    if confirm_selection:
                        ...
                        if safe_password != "" and confirm_safe_password != "" and safe_password == confirm_safe_password and is_password_valid == True:
                            ...
                            archive_content = (QueryExecutor().complex_compund_query(
                                query=archive_content_query, list_quantity=2, params=(selected_archive, logged_user_name, logged_user_document)))
                            archive_content = (QueryExecutor().treat_complex_result(
                                archive_content, to_remove_archive_list))
                            ...
                            with col2:
                                ...
                                with st.expander(label="Novos dados", expanded=True):
                                    new_archive_name = st.text_input(
                                        label="Novo nome do arquivo", max_chars=100, help="É necessário definir um nome para identificação e consulta posterior.")
                                    new_uploaded_file = st.file_uploader(label="Escolha um arquivo de texto", type=[
                                                                         "txt"], help="São permitidos arquivos de texto, na extensão '.txt'. O tamanho do arquivo não pode exceder 200 MB.")
                                ...
                                update_archive_button = st.button(
                                    label=":arrows_counterclockwise: Atualizar arquivo")
                                ...
                                if new_archive_name != "" and content is not None:
                                    ...
                                    if is_archive_name_available:
                                        with col1:
                                            with st.expander(label="Validação dos dados", expanded=True):
                                                st.success(
                                                    body="Nome do arquivo disponível.")

                                        archive_query = "INSERT INTO arquivo_texto (nome_arquivo, conteudo, usuario_associado, documento_usuario_associado) VALUES (%s, %s, %s, %s)"
                                        archive_values = (
                                            new_archive_name, content, logged_user_name, logged_user_document)
                                        ...
                                        if content is not None:
                                            ...
                                            log_query = """INSERT INTO logs_atividades (usuario_log, tipo_log, conteudo_log) VALUES(%s, %s, %s)"""
                                            log_query_values = (
                                                logged_user, "Atualização", "Atualizou os dados do arquivo {}.".format(new_archive_name))
                                            QueryExecutor().insert_query(log_query, log_query_values,
                                                                         "Log gravado.", "Erro ao gravar log:")
                        ...
                        if safe_password != "" and confirm_safe_password != "" and confirm_safe_password == safe_password and is_password_valid == True:
                            ...
                            if confirm_data_deletion and delete_archive_button:
                                ...
                                with col2:
                                    ...
                                    QueryExecutor().insert_query(delete_archive_query, archive_values,
                                                                 "Arquivo deletado com sucesso!", "Erro ao deletar arquivo:")

                                    log_query = """INSERT INTO logs_atividades (usuario_log, tipo_log, conteudo_log) VALUES(%s, %s, %s)"""
                                    log_query_values = (
                                        logged_user, "Exclusão", "Excluiu o arquivo {}.".format(selected_archive))
                                    QueryExecutor().insert_query(log_query, log_query_values,
                                                                 "Log gravado.", "Erro ao gravar log:")
