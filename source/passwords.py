...
class Passwords:
    ...
    def check_if_account_name_already_exists(
        ...
        WHERE
            ...
            AND
                usuario_associado = %s
            AND documento_usuario_associado = %s;
        """
    ...
    def get_user_passwords_quantity(self):
        ...
        user_passwords_quantity = QueryExecutor().simple_consult_query(
            check_user_passwords_quantity_query,
            params=(logged_user, logged_user_password)
        )
    ...
    def read_password(self):
        ...
        (
            logged_user_name,
            logged_user_document
        ) = Login().get_user_data(return_option="user_doc_name")
        logged_user, logged_user_password = Login().get_user_data(
            return_option="user_login_password"
        )
            ...
            if confirm_selection:
                ...
                if (
                    ...
                    with col2:
                        with st.spinner(text="Aguarde..."):
                            sleep(0.5)
                        st.subheader(body=":white_check_mark: Dados atualizados")
                    ...
                    with col2:
                        st.subheader(body=":white_check_mark: Validação")
                        with st.expander(
                            label="Validação dos dados",
                            expanded=True
                        ):
            ...
            if confirm_selection:
                ...
                if (
                    ...
                    with col2:
                        with st.spinner(text="Aguarde..."):
                            sleep(0.5)
                        st.subheader(body=":white_check_mark: Confirmação de Exclusão")
                        with st.expander(label="Dados", expanded=True):
                            aux_string = ''
                            for i in range(0, len(result_list)):
                                st.write(field_names[i])
