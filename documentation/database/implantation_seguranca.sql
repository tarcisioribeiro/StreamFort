...
CREATE TABLE `arquivo_texto` (
  ...
  KEY `fk_arquivo_texto_usuarios` (
    `usuario_associado`,
    `documento_usuario_associado`
  ),
  CONSTRAINT `fk_arquivo_texto_usuarios` FOREIGN KEY (
    `usuario_associado`,
    `documento_usuario_associado`
  ) REFERENCES `usuarios` (`nome`, `documento_usuario`) ON DELETE RESTRICT
...
CREATE TABLE `contas_bancarias` (
  ...
  UNIQUE KEY `unq_contas_bancarias` (
    `documento_proprietario_conta`,
    `instituicao_financeira`,
    `agencia`,
    `numero_conta`
  ),
  KEY `fk_contas_bancarias_usuarios` (
    `nome_proprietario_conta`,
    `documento_proprietario_conta`
  ),
  CONSTRAINT `fk_contas_bancarias_usuarios` FOREIGN KEY (
    `nome_proprietario_conta`,
    `documento_proprietario_conta`
  ) REFERENCES `usuarios` (`nome`, `documento_usuario`) ON DELETE RESTRICT
...
CREATE TABLE `senhas` (
  ...
  UNIQUE KEY `unq_senhas` (`nome_site`, `url_site`, `nome`, `login`, `senha`),
  KEY `fk_senhas_usuarios` (
    `usuario_associado`,
    `documento_usuario_associado`
  ),
  CONSTRAINT `fk_senhas_usuarios` FOREIGN KEY (
    `usuario_associado`,
    `documento_usuario_associado`
  ) REFERENCES `usuarios` (`nome`, `documento_usuario`)
...
;
-- Dump completed on 2024-11-15 18:20:26