...
class Documents:
    ...
    def validate_credit_card(self, card_number: str) -> bool:
        ...
        Returns
        -------
        is_card_number_valid : bool
            Se o cartão é válido ou inválido.
        """
        is_card_number_valid = True
        total = 0
        ...
        return is_card_number_valid

    def validate_owner_document(self, owner_document: str) -> bool:
        """
        ...
        if len(owner_document) != 11:
            ...
            for i in range(0, 9):
                second_sum_value += (
                    int(owner_document_list[i]) * second_counter
                )
            ...
            if first_division_rest == int(
                ...
                st.success(body=f"O documento {owner_document} é válido.")
                is_document_valid = True
            else:
                st.error(body=f"O documento {owner_document} é inválido.")
